...
- source of truth หลักของเว็บคือ `src/content/lodge/chapters/*.md`
- `hvitveldt_lodge.txt` ใช้เป็น reference เพื่อตรวจตกหล่นหรือเทียบ context เท่านั้น ห้ามใช้แทนเนื้อหาเว็บโดยไม่ตรวจ diff
- prompt ภาพประกอบอยู่ที่ `public/assets/lodge/prompts/hvitveldt-image-prompts.md`
- ภาพจริงอยู่ที่ `public/assets/lodge/images/backgrounds`, `public/assets/lodge/images/chapters`, และ `public/assets/lodge/images/characters`
...
4. Web QA skill
   ...
   - ต้อง preview/smoke test หน้า `/`, `/lodge/`, `/lodge/chapters/`, `/lodge/chapters/01-arrival/`, `/lodge/characters/`, `/lodge/puzzle/`
...
งานเฟสถัดไปหลัง recheck เดิมผ่านแล้ว:
- งานชุดนี้ต้องทำหลังจากแกนหลักด้านเนื้อหา, prompt alignment, page blank, puzzle gate, Character page, asset path, snow protection, และ build QA ผ่านแล้วเท่านั้น ห้ามข้ามไปทำลูกเล่น UI ก่อนแก้ฐานเดิม
- อัปเกรดระบบ `อ่านต่อจากจุดล่าสุด` ให้เมื่อผู้ใช้กดกลับมาอ่านต่อ ระบบ scroll ไปตำแหน่งล่าสุดแล้วมี animation แสงไฮไลท์เล็กน้อยบริเวณย่อหน้าหรือบล็อกข้อความที่อ่านค้างไว้ เพื่อให้ผู้อ่านจับตำแหน่งได้ทันที
- highlight ต้องสุภาพและจางลงเอง ห้ามทำเป็นเส้น tracking สีแดงหรือแถบค้างที่บังหน้าจอ ห้ามกระพริบแรง ห้ามทับตัวอักษรจนอ่านยาก และต้องเคารพ `prefers-reduced-motion`
- ถ้าตำแหน่งล่าสุดชี้ไปใกล้ภาพประกอบ chapter ให้ highlight เฉพาะบริเวณข้อความหรือ container ที่เหมาะสม ไม่ให้เอฟเฟกต์ไปทับรายละเอียดภาพ 01A/01B หรือภาพ chapter ในอนาคต
- เพิ่มระบบเน้นรายละเอียดประโยคให้เว็บอ่านเหมือนนิยายจริงมากขึ้น เช่นตัวหนา, ตัวจาง, สี accent อ่อน ๆ, หรือ emphasis สำหรับคำสำคัญในบรรยากาศ/ความคิด/หลักฐาน แต่ต้องใช้แบบมีเหตุผลจากเนื้อเรื่อง ไม่ใช่ตกแต่งสุ่ม
- การเน้นข้อความต้องไม่เปลี่ยนความหมายเดิม ไม่เพิ่มเฉลย ไม่ทำให้ประโยคไทยแตก และต้องแยกให้ชัดว่าเป็น typography/emphasis ไม่ใช่ rewrite เนื้อเรื่อง
- ถ้าต้องเพิ่ม syntax/shortcode/class สำหรับ emphasis ให้ตั้งชื่อ semantic เช่น `clue`, `memory`, `whisper`, `danger`, `cold-detail` และต้องมี CSS ที่อ่านง่ายทั้ง desktop/mobile
- ก่อนส่งงานเฟสนี้ต้องทดสอบ resume reading flow จริง: อ่านบท, เลื่อนตำแหน่ง, reload/กลับเข้าบท, กดอ่านต่อ, เห็น highlight ชั่วคราว, ไม่มี console error, ไม่มี layout shift แรง, และไม่มีหิมะทับพื้นที่อ่านหรือภาพ
